...
class MLP:
    def __init__(
        ...
        self.input_weights: list(
            ...
            []
        )  # list of lists, position 0 corresponds to the weights connecting input 1 to hidden layer 1
        self.hidden_weights: list(
            list(list(float))
        ) = (
            []
        )  # List of lists of lists, position [0][0][0] corresponds to the weights connecting hidden layer 1, node 1 to the next hidden layer node 1
        self.output_weights: list(list(float)) = []  #
        self.activation_function = activation_function
        self.chromosome = chromosome
        if chromosome == None:
            # self.init_weights(random_initialization)
            self.init_chromosome(random_initialization)
    ...
    def init_weights(self, random_flag):
        ...
        self.input_weights = [
            [
                random.random() if random_flag else 0.5
    ...
    def init_chromosome(self, random_flag):
        self.chromosome = [
            random.random() if random_flag else 0.5
            for _ in range(
    ...
    def init_weights_with_chromosome(self, chromosome):
        assert len(chromosome) == (self.n_input * self.n_hidden) + (
            (self.n_hidden * self.n_hidden) * (self.hidden_layers - 1)
        ) + (
            self.n_hidden * self.n_output
        ...
        for hidden_layer_idx in range(self.hidden_layers - 1):
            self.hidden_weights.append([])
            for hidden_node in range(self.n_hidden):
        ...
        # Finally the last hidden layer to the output node(s)
        for hidden_node in range(self.n_hidden):
            stop = start + self.n_output
    ...
    def evaluate_input(self, input_values):
        ...
        hidden_layer = self.calculate_node_values(
            input_values, self.n_hidden, self.input_weights, self.activation_function
        )
        ...
        output_layer = self.calculate_node_values(
            hidden_layer, self.n_output, self.output_weights, self.activation_function
        )
    ...
    def calculate_node_values(self, input_values, n_nodes, weights, activation):
        ...
        for node_idx in range(n_nodes):
            ...
            for input_idx in range(len(input_values)):
                ...
                values[node_idx] += (
                    input_values[input_idx] * weights[input_idx][node_idx]
                )
